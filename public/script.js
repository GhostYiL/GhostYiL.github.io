...
ws.onmessage = function(evt) {
  ...
  switch (args[0]) {
    ...
    case "id":
      ...
      document.getElementById("serverStatus").innerText =
        "Server Status: Received code from server";
      hasCode = true;
      break;
    case "joined":
      ...
      active = true;
      document.getElementById("serverStatus").innerText =
        "Server Status: Control Client joined!";
      break;
    case "left":
      //controllerClient left, moved back to waiting room
      active = false;
      document.getElementById("serverStatus").innerText =
        "Server Status: Control Client left!";
      break;
  }
};
...
ws.onclose = function() {
  document.getElementById("serverStatus").innerText =
    "Server Status: Disconnected from server";
};
...
(function loop() {
  if (myToys.size != 0) searchForToys();
...
function toggle(button) {
  ...
  if (toy.share) {
    ...
    if (active) ws.send("remove " + toy.name + ":" + id);
  } else {
    toy.share = true;
    document.getElementById(id).style.backgroundColor = "#33a151";
    if (active) ws.send("add " + toy.name + ":" + id);
  }
...
function fixToolTip(id) {
  ...
  document.getElementById(id).childNodes[1].innerHTML =
    "Sharing:" +
    (toy.share == true ? "on" : "off") +
    "<br>Status: " +
    (toy.status == 1 ? "Active" : "Inactive") +
    "<br>ID: " +
    id +
    "<br>Host: " +
    toy.url;
